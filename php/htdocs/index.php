...
if (! is_readable($config_file)) {
	...
	echo '<html>';
	echo '<head>';
	printf('<title>userman - %s</title>',pla_version());
	echo '<link type="text/css" rel="stylesheet" href="css/style.css" />';
	echo '</head>';

	echo '<body>';
	echo '<h3 class="title">Configure userman</h3>';
	echo '<br /><br />';

	echo '<center>';
	printf(_('You need to configure userman. Edit the file "%s" to do so. An example config file is provided in "%s.example".'),$config_file,$config_file);
	echo '</center>';
...
if ($pagetitle = $config->GetValue('appearance','page_title'))
	printf('<head><title>userman (%s) - %s</title></head>',pla_version(),$pagetitle);
else
	printf('<head><title>userman - %s</title></head>',pla_version());
...
function check_config() {
	...
	if ($str) {
		...
		echo '<html>';
		echo '<head>';
		echo '<title>userman Config File Error</title>';
		echo '<link type="text/css" rel="stylesheet" href="css/style.css" />';
		echo '</head>';
	...
	if (strcmp(phpversion(),REQUIRED_PHP_VERSION) < 0) {
		pla_error(sprintf('userman requires PHP version %s or greater. You are using %s',
